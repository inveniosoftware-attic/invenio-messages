...
def check_if_need_to_delete_message_permanently(msg_ids):
    ...
    """
    if not((isinstance(msg_ids, list)) or (isinstance(msg_ids, tuple))):
        msg_ids = [msg_ids]
...
def get_uids_from_nicks(nicks):
    ...
    """
    # FIXME: test case
    if not((isinstance(nicks, list)) or (isinstance(nicks, tuple))):
        nicks = [nicks]
    users = {}
...
def get_nicks_from_uids(uids):
    ...
    """
    if not((isinstance(uids, list)) or (isinstance(uids, tuple))):
        uids = [uids]
    users = {}
...
def get_uids_from_emails(emails):
    ...
    """
    # FIXME: test case
    if not((isinstance(emails, list)) or (isinstance(emails, tuple))):
        emails = [emails]
    users = {}
...
def get_gids_from_groupnames(groupnames):
    ...
    """
    # FIXME: test case
    if not((isinstance(groupnames, list)) or (isinstance(groupnames, tuple))):
...
def get_uids_members_of_groups(gids):
    ...
    """
    if not((isinstance(gids, list)) or (isinstance(gids, tuple))):
        gids = [gids]
...
def send_message(uids_to, msgid, status=CFG_WEBMESSAGE_STATUS_CODE['NEW']):
    ...
    """
    if not((isinstance(uids_to, list)) or (isinstance(uids_to, tuple))):
        uids_to = [uids_to]
    user_problem = []
    if len(uids_to) > 0:
...
def get_nicknames_like(pattern):
    ...
    if pattern:
        try:
            res = run_sql("""SELECT nickname FROM "user" WHERE nickname """ +
                          rlike_op +  """ %s""", (pattern,))
...
def get_groupnames_like(uid, pattern):
    ...
    if pattern:
        ...
        query1 = "SELECT id, name FROM group WHERE name " + rlike_op + \
